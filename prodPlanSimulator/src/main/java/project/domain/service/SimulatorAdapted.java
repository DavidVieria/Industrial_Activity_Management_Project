...
public class SimulatorAdapted {
    ...
    public void simulateWithoutPriority(List<Article> articles, List<WorkStation> workStations) {
        ...
        do {
            ...
            processCompletedEvents(workStations);

            for (Article article : articles) {

                if (!article.isInOperation() && !article.isAllOperationsCompleted()) {

                    processEvent(article, workStations);
    ...
    private void processCompletedEvents(List<WorkStation> workStationList) {
        ...
        while (!eventsQueue.isEmpty() && eventsQueue.peek().getEndTime() == currentTime) {
            ...
            for (WorkStation equalWorkStation : workStationList) {
                if (equalWorkStation.getWorkStationID().equals(workStation.getWorkStationID())) {
                    equalWorkStation.setAvailable(true);
                }
            }
    ...
    private void processEvent(Article article, List<WorkStation> workStationList) {
        ...
        if (!availableWorkStations.isEmpty()) {
            ...
            // Changes to the simulator:
            for (WorkStation equalWorkStation : workStationList) {
                if (equalWorkStation.getWorkStationID().equals(assignWorkStation.getWorkStationID())) {
                    equalWorkStation.setAvailable(false);
                }
            }
